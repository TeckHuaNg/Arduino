...
void loop() { //The loop function runs forever.
  ...
  for (int x = 0; x <= 255; x++) { //Fade in red. set x=0, if x<255, enter the loop, x++ increment add 1 to x
    analogWrite(RGBRedPin, x);//(pin num, brightness)
    delay(fadeDelay);
  }
  ...
  for (int x = 255; x >= 0; x--) { //Fade out red, green and blue.
    ...
    analogWrite(RGBBluePin, x);
    delay(fadeDelay);
    //to make orange colour, change value of data(255, 165,0)
  }
}
